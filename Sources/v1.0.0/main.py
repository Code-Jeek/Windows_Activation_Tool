...
menu_button_data = {
    ...
    3 : "self.form_view_licenses_button"
    }
window_data = {
    1 : [
...
class MainWindow():
    ...
    def __init__(self):
        ...
        self.root = ttk.Window(
            title="Windows激活工具",
            iconphoto=r".\images\icon\icon.png",
            size=(900, 600),
            resizable=(False, False),
            themename="darkly"
            )
        self.root.place_window_center()  # 设置窗口居中

        # 创建主菜单窗口组件
        self.menu_background = ttk.Frame(bootstyle=DARK, width=300, height=600)
        self.image = ttk.PhotoImage(file=r".\images\Windows_logo.png")
        ...
        self.other_messages_link = ttk.Button(text="找不到我的相关配置?", bootstyle=(INFO, LINK))
    ...
    def activation(self):
        ...
        # 获取相关信息
        key = str(self.Windows_keys_combobox.get())
        kms = str(self.Windows_kms_combobox.get())

        activation_thread = threading.Thread(target=activing, args=(key, kms))  # 创建激活进程
        activation_thread.start()  # 开始进程
    ...
    def view_licenses(self):
        ...
        view_licenses_thread = threading.Thread(target=viewing_licenses)  # 创建激活进程
